...
class VerletIntegrator(Integrator):
    ...
    def initialize_state(self, initial_position_state, initial_velocity_state):
        ...
        self.position_state_history = [initial_position_state,]
        self.velocity_state_history = [initial_velocity_state,]
    
class VerletQuaternionIntegrator(Integrator):
    ...
    def step(self, dt, acceleration):
        if len(self.position_state_history) == 1:
            ...
            xn = quaternion.from_float_array(self.position_state_history[-1].T)
            xnminus1 = quaternion.from_float_array(self.position_state_history[-2].T)
            q_omega = quaternion.from_float_array([0, ])
            xnplus1_q = xn * xnminus1 * xn * at2
            delta_q = (1/xnminus1)*xnplus1_q
            vnplus1 = quaternion.as_rotation_vector(delta_q).T/dt
            xnplus1 = quaternion.as_float_array(xnplus1_q).T
        self.position_state = xnplus1
        self.velocity_state = vnplus1
        self.position_state_history.append(self.position_state)
        self.velocity_state_history.append(self.velocity_state)
