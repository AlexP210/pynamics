...
class RevoluteMotor(JointDynamicsParent):
    def __init__(
        ...
        super().__init__(name=name, joint_names=[joint_name,])
    ...
    def update(self, topology:Topology, dt:float):
        ...
        di_dt = ((self.voltage - self.electromotive_constant*joint.get_configuration_d()[0,0]) - self.resistance * self.current) / self.inductance
